...
mode = st.sidebar.radio("モード選択", ["🏠 市場ダッシュボード", "💎 お宝発掘 (一括採点)", "🔍 個別詳細分析"])
...
st.sidebar.markdown("---")
st.sidebar.info("Ver 12.2: Auto-Detect 1.5")
...
def get_model(key):
    try:
        ...
        # 1. 現在利用可能なモデル名を全て取得する
        all_models = [m.name for m in genai.list_models() if 'generateContent' in m.supported_generation_methods]
        
        # 2. "1.5-flash" が含まれるモデルを探す (これが本命)
        target_model = next((m for m in all_models if "1.5-flash" in m), None)
        
        # 3. なければ "gemini-pro" (安定版) に逃げる
        # ※ ここで "2.5" や "2.0" を誤って選ばないように除外する
        if not target_model:
...
# --- メイン UI ---
st.title("🦅 Deep Dive Investing AI Pro (Ver 12.2)")
...
st.sidebar.markdown("---")
st.sidebar.info("Ver 12.1: Force 1.5 Flash")
...
def get_model(key):
    try:
        genai.configure(api_key=key)
        # 【重要修正】2.5などの最新版を避け、安定版の1.5を強制的に指定する
        # 自動検索をやめて、名前決め打ちで指定
...
# --- メイン UI ---
st.title("🦅 Deep Dive Investing AI Pro (Ver 12.1)")
...
# ==========================================
    ...
    if st.session_state['target_code']:
        code = st.session_state['target_code']
        model = get_model(api_key) # ここで1.5-flashが呼ばれる
        now_str = get_current_time_jst().strftime("%Y-%m-%d %H:%M")
