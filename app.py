...
st.sidebar.markdown("---")
st.sidebar.info("Ver 14.1: Strict Code Filter")
...
def get_news(code, name):
    ...
    # 検索クエリ: コードを""で囲んで完全一致を狙う
    # site指定で株探などの信頼できるソースに絞る
    queries = [
        f'site:kabutan.jp "{code}" 決算',
        f'site:kabutan.jp "{code}" 修正',
        f'site:nikkei.com "{code}" 業績',
        f'"{code}" {name} 決算発表 {this_year}',
    ]
    
    for q in queries:
        try:
            results = ddgs.text(q, region='jp-jp', timelimit='d', max_results=5)
            if not results:
            ...
            if results:
                for r in results:
                    ...
                    # 【重要】コードを含まない記事はゴミとして捨てる
                    # 6702を探しているのに6755(ゼネラル)だけしか書いてない記事は除外
                    if str(code) not in title and str(code) not in body:
                        continue
    ...
    if not news_list:
        return "指定されたコードを含む直近の決算ニュースが見つかりませんでした。", []
    
    return "\n\n".join(news_list[:12]), news_list

# --- メイン UI ---
st.title("🦅 Deep Dive Investing AI Pro (Ver 14.1)")
            ...
            try:
                ...
                with t1:
                    ...
                    with st.expander("🔍 取得したニュース一覧 (クリックで確認)", expanded=False):
                        ...
                        else: st.warning("コードに一致するニュースが見つかりませんでした")
                    ...
                    if model:
                        ...
                        【重要指示】
                        1. **他社の混同厳禁**: 記事の中に「{code}」という数字が含まれているか、銘柄名が正しいかを確認してから分析すること。
                        2. **日付の確認**: 記事の日付が現在日時({now_str})に近いもの（直近数日）を優先し、1年以上前の古い情報は「参考外」として扱うこと。
                        3. **決算分析**: 直近の決算発表があれば、その数値（売上・利益・進捗率）と市場の期待値を比較して評価せよ。
