...
# 設定・定数定義
# ==============================
st.set_page_config(page_title="新高値ブレイク分析（日本語修正版）", layout="wide")
...
def fetch_stock_data(symbol):
    ...
    # 【修正】まず辞書から名前を取得（APIの成否に関わらず辞書を優先）
    if code in JP_NAME_MAP:
        name = JP_NAME_MAP[code]
    else:
        name = code  # 辞書になければ一旦コードを入れる
    ...
    try:
        ...
        try:
            ...
            # もし辞書になく、APIで名前が取れた場合は更新
            if code not in JP_NAME_MAP:
                api_name = info.get("shortName") or info.get("longName")
                if api_name:
                    name = api_name
        except:
            # API失敗時はNoneにするが、nameは辞書の値(またはコード)を維持
            earnings_q_growth = None; trailing_eps = None; forward_eps = None
            # name = code  <-- 【修正】この行を削除 (エラー時も辞書名を維持するため)
...
# UI
# ==============================
st.title("📈 新高値ブレイク分析 (Ver 29.1)")
st.caption("オニール流シグナル判定 / 日本語名修正版 / CSV出力付")
